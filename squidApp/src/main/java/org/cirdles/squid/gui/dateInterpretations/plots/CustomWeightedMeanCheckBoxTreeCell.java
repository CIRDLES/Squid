...
import org.cirdles.squid.gui.dateInterpretations.plots.PlotsController.WeightedMeanFractionNode;
...
public class CustomWeightedMeanCheckBoxTreeCell<T> extends CheckBoxTreeCell<T> {
    ...
    private static Map<Integer, String> sortNextLabels;

    static {
        sortNextLabels = new TreeMap<>();
        sortNextLabels.put(-1, "by Time");
        sortNextLabels.put(0, "ascending");
        sortNextLabels.put(1, "descending");
    }
    ...
    private HBox sampleWeightedMeanToolbox(SampleNode sample, CustomWeightedMeanCheckBoxTreeCell<T> sampleCheckBox) {
        ...
        Button sortAgeButton = new Button("Sort " + sortNextLabels.get(currentPreferredViewSortOrder));
        ...
        sortAgeButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent e) {
                ...
                ((Button) e.getSource()).setText("Sort " + sortNextLabels.get(preferredViewSortOrder));
        ...
        Button sortRatioButton = new Button("Sort " + sortNextLabels.get(currentPreferredViewSortOrder));
    ...
    private void synchronizeCheckBoxesWithWM(SampleNode sample) {
        for (TreeItem<T> spotCheckBox : this.getTreeItem().getChildren()) {
            int indexOfSpot = ((WeightedMeanFractionNode) spotCheckBox.getValue()).getIndexOfSpot();
